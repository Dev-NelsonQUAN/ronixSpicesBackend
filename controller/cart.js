...
exports.addToCarts = async (req, res) => {
    try {
        ...
        if (!product) return res.status(400).json({message: "Product no found"});

        let cart = await cartModel({user: req.user._id});
        if (!cart) {
            cart = new cartModel({
                user: req.user._id,
...
exports.getUserCart = async (req, res) => {
    try { 
        ...
        return res.status(200).json({message: "Gotten cart", cart})
    } catch (error) {
